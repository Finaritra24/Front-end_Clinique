...
export default function ListRecette() {
  ...
  useEffect(() => {
    fetch('http://localhost:8081/listg-CategorieRecette', { credentials: 'include' })
  ...
  const handleOnClick = (id) => {
    fetch('http://localhost:8081/setCookie-CategorieRecette', {
      method: 'POST',
    ...
    const deleteClick = (id) => {
      fetch('http://localhost:8081/dropg-CategorieRecette', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id}),
        credentials: 'include'
                    ...
                    <div className="card">
                        <DataTable value={recettes} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}>
                            ...
                            <Column   field="button" header="button" style={{ width: '25%' }}
                              body={(rowData) => {
                                  return <div><Button icon="pi pi-file-edit" onClick={() => handleOnClick(rowData.idCategorieRecette)}/> <Button icon="pi pi-times" onClick={() => deleteClick(rowData.idCategorieRecette)}/></div>;
                              }} >
                            </Column>
                        </DataTable>
